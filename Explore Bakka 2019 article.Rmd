...
# Non-stationary Gaussian models with physical barriers. Haakon Bakka

### apendixC.copy

[Code replicate](https://www.sciencedirect.com/science/article/pii/S221167531830099X)

```{r apendixC.copy, eval=FALSE}
spde = inla.spde2.pcmatern(mesh,prior.range = c(6, .5), prior.sigma = c(3, 0.01))
formula = y -1+m + f(s, model=spde) + f(iidx, model="iid", hyper=hyper.iid)

#while for MB we replace that code with
barrier.model = inla.barrier.pcmatern(mesh, barrier.triangles, prior.range = c(6, .5), prior.sigma = c(3, 0.01))
formula = y -1+m + f(s, model=barrier.model) + f(iidx, model="iid", hyper=hyper.iid)

#where barrier.triangles is a list of indices of the mesh triangles covering land.
```

### B.copy

[Code replicate](https://haakonbakkagit.github.io/btopic128.html#3_Case_study)
...
```{r B.copy all models, eval=FALSE}
## Overall list
M = list()
...
```{r B.preBarrierModel.copy, eval=FALSE}
...
```{r B.BarrierModel.copy, eval=FALSE}
...
Plotting Priors
In this section we show how to obtain some nice plots which highlight the difference between the stationary model and the Barrier model (as prior models, before they are fitted to data).
```{r B.BarrierModelQ.copy, eval=FALSE}
## Before anything else we define a function to find the autocorrelatiions given a precision matrix Q, a location and a mesh.
...
```{r B.BarrierModelPriorPlotfx.copy, eval=FALSE}
...
```{r B.BarrierModelPriorPlot.copy, eval=FALSE}
local.plot.field(corr, mesh, zlim=c(0.1, 1))
points(8.01, 9.97)

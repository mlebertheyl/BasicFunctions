...
local.plot.field = function(field, ...){
  ...
  # - Do the projection
  image.plot(list(x = proj$x, y=proj$y, z = field.proj), 
             xlim = xlim, ylim = ylim, col = plasma(50), ...)  
  # - Use image.plot to get nice colors and legend
}
print(mesh$n)
# - This is the appropriate length of the field variable
...
# this is for the non barrier field
```
...
```{r}
# alternative plot in case I wanna put it inside the functions
...
```{r}
my_diff.pos <- function(mesh, loc.data, 
                        barrier.model, Q, u,
         sigma.u = 1, sigma.epsilon = 0.2,
         range.fraction = c(0.1, 0.1), # needs to be the same as mydiff.r
...
posmean <-  my_diff.pos(mesh, loc.data, data4pos$barrier.model,
              data4pos[[2]],
              data4pos$sample, 
              range.fraction = c(0.1, 0.1),
              return.list = FALSE, return.plot = TRUE)
posmean2 <-  my_diff.pos(mesh, loc.data, data4pos2$barrier.model,
              data4pos2[[2]],
              data4pos2$sample, 
              range.fraction = c(0.1, 0.8),
              return.list = FALSE, return.plot = TRUE)
...
```{r}
# SPATIAL TRUE FIELD AND POSTERIOR FOR BARRIER MODEL
#####
nfrac <- 20
################ dir_out
...
for(r in 1:nfrac) {
  ...
  mats <- mydiff.r(mesh, fem = mat.plus, barrier.triangles = list(bar1, bar2),
                   ...
                   return.list = TRUE, return.plot = TRUE)
  plot(poly.bar.orginal, add=T) # see if it works, pos function has it inside
  dev.off()
  ####  posterior mean gif
  fpp <- file.path(dir_out_pm, paste0(f, ".png"))
  png(fpp)
  my_diff.pos(mesh, loc.data, mats$barrier.model,
              mats[[2]],
              mats$sample, 
              range.fraction = c(0.05, f),
              return.list = FALSE, return.plot = TRUE)
...
imgs <- list.files(dir_out_f, full.names = TRUE)
...
true.field.fps2.r20 <- image_animate(img_joined, fps = 2)
true.field.fps1.r20 <- image_animate(img_joined, fps = 1)
# this means we are plotting the true field with x fps and range fraction 20

## save to disk
image_write(image = true.field.fps2.r20,
            path = "true.field.fps2.r20.gif")
image_write(image = true.field.fps1.r20,
            path = "true.field.fps1.r20.gif")

# POSTERIOR MEAN
## list file names and read in
imgs <- list.files(dir_out_pm, full.names = TRUE)
img_list <- lapply(imgs, image_read)
...
## animate at 2 frames per second
pos.mean.fps2.r20 <- image_animate(img_joined, fps = 2)
pos.mean.fps1.r20 <- image_animate(img_joined, fps = 1)

## view animated image
# img_animated_pm

## save to disk
image_write(image = pos.mean.fps2.r20,
            path = "pos.mean.fps2.r20.gif")
image_write(image = pos.mean.fps1.r20,
            path = "pos.mean.fps1.r20.gif")
...
I need to run things that are common for stationary and barrier, so run everything is easier 
...
```{r}
my_diff.stat <- function(mesh, df, 
                         prior.range = c(1, 0.5),
                         prior.sigma = c(1, 0.5),
                         range.fraction,
                         pos = res.stationary$summary.random$s$mean,
                         return.list = FALSE, return.plot = TRUE) {
  model.stat <- inla.spde2.pcmatern(mesh, 
                                    prior.range = c(prior.range[1], prior.range[2]),
  ...
  if (return.list == TRUE) {
    return(list(pos = quote(pos), 
                res = res.stationary, summ.res = summary(res.stationary)))
  }
}
...
estimate.stat <- my_diff.stat(mesh, posmean$df,
                         range.fraction = c(0.1, 0.1))
```
...
for(r in 1:nfrac) {
  ...
  pos <- my_diff.pos(mesh, loc.data, mats$barrier.model,
              mats[[2]],
              mats$sample, 
              range.fraction = c(0.05, f),
              return.list = TRUE, return.plot = FALSE)
  
  #### posterior mean estimate stationary model
  fpp <- file.path(dir_out_s, paste0(f, ".png"))
  png(fpp)
  stat <- my_diff.stat(mesh, pos$df,
                       range.fraction = c(0.05, f),
                       return.list = FALSE, return.plot = TRUE)
  plot(poly.bar.orginal, add=T) # see if it works, pos function has it inside
  dev.off()
...
# TRUE FIELD
## list file names and read in
imgs <- list.files(dir_out_s, full.names = TRUE)
img_list <- lapply(imgs, image_read)
...
## animate at 2 frames per second
stat.rnadom.mean.fps2.r20 <- image_animate(img_joined, fps = 2)
stat.rnadom.mean.fps1.r20 <- image_animate(img_joined, fps = 1)
# this means we are plotting the true field with x fps and range fraction 20

## save to disk
image_write(image = stat.rnadom.mean.fps2.r20,
            path = "stat.rnadom.mean.fps2.r20.gif")
image_write(image = stat.rnadom.mean.fps1.r20,
            path = "stat.rnadom.mean.fps1.r20.gif")
```
...
Do plots for sd and quantile posteriors
